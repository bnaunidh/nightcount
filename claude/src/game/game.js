 ...
 */
const NIGHT_SCOPED = [
  "investigate", "trail", "pole", "clean", "readings_hint",
...
export class Game {
  ...
  async startNight(n, fresh, resume = null) {
    ...
    // standing, it was.
    this.torch.have = true;
    this.state.flag("has_torch", true);
  ...
  _baseInteractions() {
    ...
    const SALT_AT_DOOR = { office: "office", bath: "bath", rear: "rear", entry: "front" };
    ...
    I.add({
      ...
      id: "door_entry", pos: V(5.1, 1.2, 0), radius: 1.5,
      ...
      enabled: () => !(this.tasks.has("open") && !this.tasks.isDone("open"))
        && !this.state.flag("closing") && !this.salt?.pending?.("front"),
    ...
    for (const [name, label] of [["office", "the office door"],
      ...
      I.add({
        id: "door_" + name, radius: 2.0, label,
        pos: V((d.def.x0 + d.def.x1) / 2, 1.2, (d.def.z0 + d.def.z1) / 2),
  ...
  update(dt) {
    ...
    if (this.mode === "play" || this.mode === "cutscene") {
      ...
      if (this.mode === "play" && !this.ui.focused) {
