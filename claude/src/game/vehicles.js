...
export class Vehicle {
  ...
  async spawn() {
    ...
    const want = DIMS[key] || DIMS.car_sedan;
  ...
  _makeLights(key, dims) {
    const isTruck = key === "truck_semi" || key === "tanker";
  ...
  // ——— per-frame ————————————————————————————————————————————————
  _isTruck() {
    return this.def.vehicle === "truck_semi" || this.def.vehicle === "tanker";
  }
  ...
  _engineStop() {
    ...
    if (n && n.stop) n.stop(0.35);
    this.engine.node = null;
    this.g.audio.play("veh_car_leave", { vol: 0.22, pos: this.group.position.clone(), ref: 6 });
  }
