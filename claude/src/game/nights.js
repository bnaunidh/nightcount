...
const night1 = (g) => scriptShell(g, async (ctl) => {
  ...
  g.tasks.set([
    // 20:20 – 21:00 · PREP
    { id: "clock_in", text: "Punch in — the clock is by the office door" },
  ...
  g.chores.setCleanQueue([
    [2.6, 4.2, "shop", "in front of the counter"],
    [-1.4, 6.2, "shop", "by the west aisle"],
    [7.0, 10.5, "bath", "the restroom"],   // last, always — blood under this one
  ]);
  ...
  g.tasks.add({ id: "investigate", text: "⚠ INVESTIGATE — out by the road, north-west" });
...
const night2 = (g) => scriptShell(g, async (ctl) => {
  ...
  // ——— Night 2: the rain, the flash, and Jacob ————————————————————
  if (!g.resuming && !g.skipIntro) await g.cutscene("cs02_jacob", async (cs) => {
    await cs.fade(1, 0);
    g.player.teleport(V(6.5, 0, -14.5), Math.PI);
    g.audio.subtract("amb_rain_heavy", false, 1.0);
    g.audio.play("veh_car_arrive", { vol: 0.45 });
    await cs.fade(0, 2.2);

    cs.say("—and it's gonna rain all week, so don't forget your umbrellas, folks—",
      { who: "RADIO", secs: 4.2 });
    await cs.wait(2.7);
    g.audio.play("int_radio_static", { vol: 0.4 });
    cs.say("Oh, come on. All week?", { who: "", secs: 2.8 });
    await cs.wait(1.9);

    // CRACK. A sound that is not thunder: wrong shape, wrong direction.
    g.audio.play("amb_thunder", { vol: 0.95, scare: true });
    g.player.shake(0.6, 0.9);
    await cs.wait(0.6);
    // one frame of him, standing in the rain, covered in blood
    await g.attendant.flashAt?.(V(0.5, 0, -12.0), 0.14);
    g.audio.play("hor_static_burst", { vol: 0.5 });
    await cs.wait(1.1);

    cs.say("…", { who: "", secs: 1.4 });
    await cs.wait(1.0);
    cs.say("JACOB???", { who: "", secs: 2.6 });
    await cs.wait(1.7);

    g.audio.play("veh_car_door", { vol: 0.55 });
    cs.say("Umbrella. Flashlight.", { who: "", secs: 2.4 });
    await cs.wait(1.6);
    g.player.teleport(V(4.2, 0, -6.0), Math.PI);
  });
  if (ctl.aborted) return;
  ...
  if (!g.resuming && !g.skipIntro) {
    await g.torch.take();
    g.torch.toggle(true);
    g.tasks.add({ id: "trail", text: "⚠ INVESTIGATE JACOB SIGHTING — NOW" });
    g.station.showForecourtBlood?.(true);
    g.ui.say("Blood. Going thin in the rain, but it's there.", { who: "", secs: 4 });
    g.state.clue("trail_n2", "A blood trail across the forecourt, leading inside");
    g.setDread(3);

    // it leads in through the shop, down the hall, into the restroom
    await Promise.race([g.tasks.wait("trail"), wait(180)]);
    if (ctl.aborted) return;
    ...
    await g.cutscene("cs02b_bathroom", async (cs) => {
      ...
      await cs.wait(0.3);
      g.attendant.hide();                       // the flash fills the window, and he is gone
      await cs.wait(0.6);

      // everything goes black. THUD.
      await cs.fade(1, 0.18);
      g.audio.play("hor_thump_wall", { vol: 0.8, scare: true });
      await cs.wait(3.4);                       // several seconds of nothing
      g.audio.play("hum_breath", { vol: 0.6 });
      await cs.wait(1.6);
      await cs.fade(0, 1.8);
      g.player.teleport(V(7.0, 0, 10.2), 0);

      cs.say("…ow.", { who: "", secs: 2.0 });
      await cs.wait(2.4);
      cs.say("Okay. Okay.", { who: "", secs: 2.2 });
      await cs.wait(2.4);
      cs.look(V(7.2, 1.4, 11.9), 0.8);
      cs.say("There's nothing there. There's nothing there.", { who: "", secs: 3.4 });
      await cs.wait(3.6);
      cs.look(V(7.0, 0, 10.4), 1.0);
      await cs.wait(1.4);
      cs.say("…Then why is it still warm.", { who: "", secs: 3.8 });
      await cs.wait(4.0);
      cs.say("Clean it. Just clean it. Then do the job.", { who: "", secs: 3.6 });
      await cs.wait(3.8);
    });
    g.tasks.done("trail");
    g.state.clue("warm_n2", "The blood in the restroom was still warm");
    g.state.note("It was warm. I put my hand in it and it was warm.", g.minutes);
    g.setDread(4);
  }

  g.tasks.set([
    { id: "clock_in", text: "Punch in" },
    { id: "note", text: "Read the manager's note — it's on the register" },
  ...
  await g.attendant.showAt(3, { stage: 1, anim: "Crouch_Idle_Loop" });
...
const night3 = (g) => scriptShell(g, async (ctl) => {
  ...
  await g.chores.makeSpill(2.2, 4.6);
  g.audio.play("hor_bottle_fall", { vol: 0.55, pos: V(2.2, 0.3, 4.6), scare: true });
  ...
  if (ctl.aborted) return;
  {
    ...
    g.setDread(5);
    // deliberately no task. The list does not tell him to look.
    ...
    const wentOut = await Promise.race([
      new Promise((res) => {
        const stop = g.timers.every(0.4, () => {
          if (g.player.pos.z < -3.2) { stop(); res(true); }
        });
...
const night4 = (g) => scriptShell(g, async (ctl) => {
  ...
  await g.cutscene("cs04_sleep", async (cs) => {
    ...
    for (let i = 0; i < 5; i++) {
      g.audio.play("step_lino", { vol: 0.32, rate: 0.94 + i * 0.01 });
      g.audio.play("step_lino", { vol: 0.20, rate: 1.42, delay: 0.17 });
      await beat(0.52);
    }
    ...
    for (let i = 0; i < 4; i++) {
      g.audio.play("step_lino", { vol: 0.34, rate: 0.95 });
      await beat(0.5);
    }
    ...
    for (let i = 0; i < 6; i++) {
      g.audio.play("step_lino", { vol: 0.30 - i * 0.03, rate: 0.95 });
      await beat(0.5);
    }
    g.setClock(0, 21);
...
const night5 = (g) => scriptShell(g, async (ctl) => {
  ...
  for (let i = 0; i < 7; i++) {
    g.audio.play("step_lino", { vol: 0.18 + i * 0.09, rate: 1.5 });
    await wait(0.26);
  }
...
const night6 = (g) => scriptShell(g, async (ctl) => {
  ...
  (async () => {
    for (let i = 0; i < 60 && !g.tasks.isDone("getout") && !ctl.aborted; i++) {
      g.audio.play("step_lino", { vol: 0.3, rate: 0.8 });
