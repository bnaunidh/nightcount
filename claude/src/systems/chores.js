...
export class ChoreSystem {
  ...
  async take(kind, modelKey, h = 0.35) {
    ...
    const o = await model(modelKey, { shadows: false });
    fitHeight(o, h);
    o.position.set(0.16, -0.26, -0.42);
    o.rotation.set(0.2, 0.6, 0);
    this.g.camera.add(o);
    this.held = { kind, object: o };
    // Four different objects were sharing two sounds — a mop and a bin bag both
    // came up as a glass bottle. Each thing you can pick up now sounds like
    // itself, which is also the only cue that tells you what is in your hands
    // when it is off the bottom of the screen.
    const PICKUP = {
      box: "int_box_cardboard", mop: "int_mop_bucket",
      trash: "int_trashbag", broom: "int_broom",
    };
    this.g.audio.play(PICKUP[kind] || "int_bottle", { vol: 0.5 });
    this.g.player.speedScale = kind === "box" ? 0.78 : 0.92;
    this.g.ui.toast(HELD_LABEL[kind] || "");
    return this.held;
  }

  drop() {
    if (!this.held) return;
    this.g.camera.remove(this.held.object);
    this.held = null;
    this.g.player.speedScale = 1;
  }
  ...
  _register() {
    ...
    I.add({
      id: "punch", pos: V(-8.4, 1.5, 7.6), radius: 1.8,
      label: "the time clock", verb: "Punch",
      onUse: () => {
    ...
    I.add({
      id: "spill", pos: () => this.spill?.pos, radius: 2.0,
      label: "the spill", verb: "Mop it",
      enabled: () => !!this.spill && this.holding("mop"),
      onUse: () => this.mopSpill(),
    });
    ...
    I.add({
      id: "trashbag", pos: V(8.2, 0.5, 2.4), radius: 1.8,
    ...
    I.add({
      id: "openstation", pos: V(5.1, 1.1, 0.35), radius: 2.2,
      ...
      onUse: () => {
        ...
        g.timers.after(2.5, () => g.station.setDoor("entry", false));
    ...
    I.add({
      ...
      id: "lockfront", pos: V(5.1, 1.1, 0.35), radius: 2.2,
      ...
      enabled: () => !(g.tasks.has("open") && !g.tasks.isDone("open")
        && g.minutes < 21 * 60) && !g.salt?.pending?.("front"),
      ...
      onUse: () => {
        ...
        if (g.tasks.has("lockup") && g.state.flag("closing")) {
          ...
          this.lockedUp = true;
          g.ui.toast("Locked up. The clock's by the office door.");
        } else {
  ...
  async makeSpill(x, z, where = "shop", place = "") {
    ...
    const m = new THREE.Mesh(geo, new THREE.MeshLambertMaterial({
      color: 0x2b2f33, transparent: true, opacity: 0.75,
    }));
    ...
    this.spill = { mesh: m, pos: new THREE.Vector3(x, 0.4, z), where, place, strokes: 0 };
  ...
  clearSpill() {
    if (this.spill) this.g.scene.remove(this.spill.mesh);
    this.spill = null;
  }
  ...
  mopSpill() {
    ...
    this.g.audio.play("int_mop", { vol: 0.5 + sp.strokes * 0.06,
      rate: 0.94 + Math.random() * 0.12 });
    if (sp.strokes < 3) {
      const left = 1 - sp.strokes / 3;
      sp.mesh.material.opacity = 0.75 * left;
