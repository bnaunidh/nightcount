...
// The internal buffer is an sRGB target, so the GPU decodes it as it is
// sampled and everything above this line works in LINEAR light. The canvas is
// not linear. This pass used to write those linear values straight out, which
// put a mid-grey of 128 on screen at 59 and crushed every shadow in the game
// into the same black — measured, not guessed. Encode once, here, and do the
// grain and the dither after it, in the space the eye actually reads.
vec3 toSRGB(vec3 c){
  c = max(c, 0.0);
