...
import { actionDown, takeMouse, isLocked, isDown } from "./input.js";
import { clamp, lerp } from "./util.js";

const EYE = 1.62, RADIUS = 0.33;
const WALK = 1.45, SPRINT = 3.6;
...
export class Player {
  ...
  _hits(c, x, z) {
    const dx = x - Math.max(c.x0, Math.min(x, c.x1));
    const dz = z - Math.max(c.z0, Math.min(z, c.z1));
  ...
  _blocked(x, z) {
    ...
    for (let i = 0; i < cols.length; i++) {
      const c = cols[i];
      if (!c.blockLow && c.y1 !== undefined && c.y1 < 0.45) continue;   // step over low kerbs
      if (this._hits(c, x, z)) return true;
    }
    return false;
  }
  ...
  update(dt) {
    ...
    if (!this.frozen && this.canMove) {
      ...
      if (f || s) {
        ...
        const runKey=actionDown("sprint")||(settings.keys.sprint==='ShiftLeft'&&isDown('ShiftRight'));
        const sp=(runKey ? SPRINT : this.emergencyRun ? 2.0 : WALK)*this.speedScale;
    ...
    const steps = Math.max(1, Math.ceil(Math.hypot(this.vel.x, this.vel.z) * dt / .08));
    for (let i = 0; i < steps; i++) {
      const nx = this.pos.x + this.vel.x * dt / steps;
      const nz = this.pos.z + this.vel.z * dt / steps;
