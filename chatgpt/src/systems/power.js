...
export class PowerSystem {
  ...
  set(id, v) {
    ...
    if (v && this.load >= this.capacity) {
      ...
      this.g.audio.play(this.g.audio.has("nc_power_cut") ? "nc_power_cut" : "int_breaker",
        { vol: 0.8, scare: true });
  ...
  apply() {
    ...
    const setL = (name, on, scale = 1) => {
      const l = L[name];
      if (!l) return;
      l.intensity = on ? this._base[name] * scale : 0;
    };
    for (const n of ["fl_counter", "fl_mid", "fl_east", "fl_west", "fl_aisle_front", "fl_entry_aisle"]) setL(n, store);
    setL("front", store);
    for (const n of ["fl_bath", "fl_office", "fl_stock"]) setL(n, back);
    setL("rear", back);
    setL("canopy", canopy);
    setL("delivery", canopy);
    ...
    for (const {face,circuit} of (S.litSigns || [])) {
      const on = circuit === "store" ? store : canopy && (circuit !== "road" || !this.g.state.flag("sign_off"));
      face.material.color.setHex(on ? 0xffffff : 0x141917);
    }
    if (S.bankFaces) {
  ...
  update(dt) {
    ...
    if (this.flicker > 0) {
      ...
      for (const n of ["fl_counter", "fl_mid", "fl_east", "fl_west", "fl_aisle_front", "fl_entry_aisle"]) {
