...
import { loadSoundPack } from "./sound-pack.js";
import { prepareShotgunSounds } from "./shotgun-sound.js";
...
export class Audio {
  constructor() {
    ...
    this.rand = rng(1972);
    this._nextOneShot = 20;this.truckSilenced=false;this.truckOneShots=new Set();
    this._pending = [];
    this._onReady = [];
  ...
  async init() {
    ...
    await this.loadAll();
    await prepareShotgunSounds(this);
    await loadSoundPack(this);
    await this.loadHumanVoices();
    this.setState(this.state === "off" ? "normal" : this.state, 0.1);
    for (const f of this._pending.splice(0)) f();
    // This edition uses licensed actor recordings. Unrecorded custom dialogue
    // stays subtitled; the legacy local TTS archive is not loaded or exported.
  ...
  silenceTruckTraffic(){this.truckSilenced=true;for(const source of this.truckOneShots){try{source.stop();}catch{}}this.truckOneShots.clear();}
  async loadHumanVoices(){
    this.humanVoices={count:0};this.humanLine=new Map([['Yes.','male_yes'],['No.','male_no']]);
    try{const r=await fetch('assets/audio/human/manifest.json');const {clips}=await r.json();
      await Promise.all(clips.map(async c=>{const r=await fetch(c.file),b=await this.ctx.decodeAudioData(await r.arrayBuffer());this.buffers.set('human_'+c.id,b);this.norm.set('human_'+c.id,1);this.humanVoices.count++;}));
    }catch(error){console.warn('Human voice clips unavailable',error);}
  }
  humanReaction(id,kind='room'){
    const slot='human_'+id;if(!this.has(slot))return 0;this._lastLine?.stop?.();this._lastLine=this.voice(slot,kind,{vol:.66});return this.buffers.get(slot).duration;
  }
  ...
  speak(text, kind = "room", vol = 1.0) {
    ...
    const human=this.humanLine?.get(text);
    if(human)return this.humanReaction(human,kind);
    if (!slot) return 0;
  ...
  _layer(slot) {
    ...
    try { src.start(0, this.rand() * Math.max(0, buf.duration - 0.2)); } catch (e) { src.start(); }
    ...
    return {
      ...
      stop: (fade = 0.4) => {
        ...
        try {
          ...
          if(fade===0){g.gain.value=0;src.stop();src.disconnect();g.disconnect();p.disconnect();return;}
  ...
  /** One complete footfall; never repeat the same take consecutively. */
  step(surface, options = {}) {
    const type = surface === "concrete" || surface === "gravel" ? surface : "tile";
    if (this.soundPack?.status === "ready") {
      this._lastStep ||= {};
      const last = this._lastStep[type];
      let next = Math.floor(this.rand() * (last === undefined ? 4 : 3));
      if (last !== undefined && next >= last) next++;
      this._lastStep[type] = next;
      return this.play(`nc_footstep_${type}_${String(next + 1).padStart(2, "0")}`, {
        vol: 0.3, rate: 0.97 + this.rand() * 0.06, ...options,
      });
    }
    const slot = { tile: "step_lino", concrete: "step_concrete", gravel: "step_gravel" }[type];
    return this.play(slot, { vol: 0.3, dur: 0.26, ...options });
  }
  ...
  update(dt, camera, player) {
    ...
    const opts = ONESHOTS.filter((o) => this.dread >= o.min && this.dread <= o.max && !(this.truckSilenced&&o.pool.includes("veh_truck_pass")));
    ...
    for (const o of opts) {
      ...
      if (r <= 0) {
        ...
        const source=this.play(slot, { vol: 0.22 + this.dread * 0.045, pos, rolloff: 1.4 });
        if(source&&slot==="veh_truck_pass"){this.truckOneShots.add(source);source.addEventListener("ended",()=>this.truckOneShots.delete(source),{once:true});}
        break;
      }
    }
  }
